...
function ChatBox({ messages, socket, gameDetails, tableDetails, playerDetails }) {
        ...
        {messages?.map((msg, index) => (
          <Box
            ...
            sx={{
              ...
              width: "100%",
              display: "grid",
              gridTemplateColumns: "minmax(10px, auto) 1fr auto",
              gap: 1,
              alignItems: "center",
              px: 1,
              py: 0.5,
          ...
          >
            <Box sx={{ fontWeight: "bold", color: "#4fc3f7", textAlign: 'left', width: '1em'  }}>
              {msg.playerId}
            </Box>
            <Box sx={{background: 'pifnk', textAlign: 'left', ml: 4}}>{msg.message}</Box>
            <Box sx={{ fontSize: "0.6em", color: "rgba(255, 255, 255, 0.6)" }}>
              {msg.timestamp}
            </Box>
          </Box>
        ))}
